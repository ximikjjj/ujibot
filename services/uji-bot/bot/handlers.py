...
async def cmd_start(message: Message):
    ...
    await message.answer(
        ...
        f"Я вижу входящие сообщения и сразу подсказываю что написать: коротко, по делу, без морали.\n\n"
        f"<b>Статус:</b> {status}",
        parse_mode="HTML",
...
async def cmd_digest(message: Message):
    ...
    if not digest:
        await message.answer(
            ...
            "Он генерируется вечером на основе переписок дня."
        )
        return

    text = f"<b>Разбор за сегодня</b>\n\n"
    if digest.what_worked:
        text += f"Что получилось:\n{digest.what_worked}\n\n"
    if digest.near_misses:
        text += f"Чуть не испортил:\n{digest.near_misses}\n\n"
...
async def process_phone(message: Message, state: FSMContext):
    ...
    try:
        result = await start_auth_request_code(message.from_user.id, phone)
        await state.update_data(phone=phone)
        await state.set_state(AuthStates.waiting_code)
        await message.answer(
            "Код отправлен в Telegram.\n\n"
            "Введи код из сообщения (цифры через пробел или слитно, например <code>12345</code>):",
            parse_mode="HTML"
        )
